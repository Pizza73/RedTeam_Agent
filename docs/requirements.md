...
| LOOP-029 | Base Refresh Authorizationは1回のExpected-HEAD Branch Updateだけを認可し、ResumeまたはImplementation Requestを認可しない。Design Stop中の後続Governance取込は、Gate HEADからCurrent HEADまで最大32段の各EdgeがPrevious HEADと認可済みDefault SHAを親に持つ2親Mergeで、Previous HEADに対応するTrusted Statusがある場合だけ同じGateから再認可する。RunnerはTransition消費、最新Gate、Stop latch、Current HEADを各Remote Write後とCodex Trigger直前に再取得し、Design StopまたはAuthority Driftがあれば`ai-loop-blocked`を維持して停止する。Lifecycle Projectionだけの遅延・残存はTrusted Transitionが正規化する |
