...
| LOOP-009 | Phase GateをPASSするまで次Phaseを開始しない。自動遷移時のcurrent/next Phase labelとLoop state labelは単一の完全置換で更新し、Phase labelが0件または複数件となる中間状態を公開しない。置換前snapshot以降のlabel event差分はGate自身が生成すると期待した完全なevent集合・actorと一致しなければならず、並行label writeはFail Closedにする |
