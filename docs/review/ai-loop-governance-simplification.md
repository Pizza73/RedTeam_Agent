...
not authorize a second governance advance after its first merge output. The follow-up refresh-chain
rule requires every prior authorized merge edge rather than treating ancestry alone as authority.
After that rule is merged, PR #3 can incorporate current `main` while retaining Phase 0C and the stop
latch. Current-HEAD checks run without producing Review Ready, then the existing PR #30 design commit
and recurrence Gate can be bound to one Design Approval and one implementation request.
