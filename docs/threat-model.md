...
| Blocked PR cannot read newly merged governance and bypasses refresh evidence | Approver workflow binds the current-Phase `BLOCKED_LIMIT` gate, its unique adjacent base PASS, exact old HEAD and current default SHA; local update preserves the Phase, verifies both resulting ancestries and current checks, then uses only a gate-bound bounded Resume |
