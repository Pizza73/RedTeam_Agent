...
| Quarantine is erased while ingestion outcome exists only in memory | Durable, read-back-verified SecureIngestionManifest precedes deletion intent; erasure and final result acknowledgement have explicit recoverable states |
| Audit/key CAS advances to a generation whose local path is swapped or unreachable | External anchor CAS binds generation, state digest and immutable blob ID; restart loads the exact trusted blob and fails closed without older local alternate fallback |
