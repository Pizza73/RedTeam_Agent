...
| Runner observes a zero/multiple-Phase transition window | Replace the complete managed label set atomically from a fresh PR snapshot, audit all label events since the pre-snapshot watermark against the exact expected GitHub Actions event set/actor, then re-fetch and compare the exact HEAD, next Phase and full label set |
