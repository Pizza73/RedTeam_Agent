...
systemd単位には`IPAddressDeny=any`を残すが、現在の`IPAddressAllow=0.0.0.0/0`によりIPv4はingress/egressとも全許可となる。
制限が必要な配備ではこれを管理端末CIDR、LLM許可CIDR、固定AD Collector IPへ分割し、`vllmAllowedCidrs`以下に狭くする。
`product.json`とCollector IPは実DCへ同時に変更する。将来Impacket workerを有効化する場合は、control planeとは別のworker単位で
Target IPとTCP/445またはTCP/135だけを許可し、そのsandbox identityをPhase 5 live attestationへ結び付ける。
