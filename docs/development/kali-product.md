...
`redteam-product`は、現在のKali Linux上で認証済みオペレーターUI、永続Mission/Approval状態、Phase 2 vLLM能力検証を一つのloopbackサービスとして構成する。設定は`deployment/kali/product.json`、systemd単位は`deployment/systemd/redteam-agent.service`に固定する。
...
- UI: `127.0.0.1:18000`のみ。外部端末からはSSH port forwardingを使用する。
...
5. ローカルブラウザで`http://127.0.0.1:18000`を開き、UI tokenを一度入力する。tokenはHttpOnly session cookieへ交換され、プロセス再起動時に全sessionが失効する。
