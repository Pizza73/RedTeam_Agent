...
def registry_payload(tools: tuple[ToolDefinition, ...], registry_revision: int) -> dict[str, object]:
    return {
...
def build_registry_revision(
    *,
    ...
    ordered = tuple(sorted(tools, key=lambda item: (item.tool_ref.tool_id, item.tool_ref.registry_revision)))
    for tool in ordered:
        if tool.target_extractor_id is not None:
            extractors.resolve(tool.target_extractor_id)
        try:
            Draft202012Validator.check_schema(tool.parameter_schema.to_dict())
